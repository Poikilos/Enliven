...
if [ -z "$ENLIVEN_REPO" ]; then
    ...
    for try_enliven_repo in "$HOME/Downloads/Poikilos/Enliven" "$HOME/Downloads/EnlivenMinetest" "$try_default_enliven_repo"
    do
...
if [ -f utilities/mtbuild.rc ]; then
    cd utilities
fi
source mtbuild.rc
if [ $? -ne 0 ]; then
    printf "* trying \"$ENLIVEN_REPO/utilities/mtbuild.rc\"..."
    source "$ENLIVEN_REPO/utilities/mtbuild.rc"
    if [ $? -ne 0 ]; then
        echo "FAILED"
        echo "Error:"
