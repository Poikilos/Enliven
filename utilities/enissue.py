...
License: See license file at https://github.com/Poikilos/Enliven

This script caches issues (To
~/.cache/enissue/Poikilos/Enliven/issues
by default).
...
# https://api.github.com/repos/Poikilos/Enliven/issues/475/timeline
...
default_options = {
    # 'repo_url': "https://api.github.com/repos/Poikilos/Enliven",
    'repo_url': "https://github.com/Poikilos/Enliven",
}
...
modes = {
    "list": {
        ...
        "examples": ["list", " list not_minetest_game",
                     " list not_minetest_game urgent", " not_minetest_game",
                     " not_minetest_game urgent", " not_minetest_game --closed"]
    },
    "labels": {
    ...
    "find": {
        ...
        "examples": [
            ...
            " find mobs not_minetest_game  # term=mobs, label=not_minetest_game",
            " find mobs # term=mobs",
    ...
    "open": {
        ...
        "examples": [
            " --closed",
            " --closed --open",
            " not_minetest_game --closed  # closed, label=not_minetest_game",
            " not_minetest_game --closed open  # open, label=not_minetest_game",
        ]
    },
    "page": {
        "parent": "list",
...
# ^ The parent mode is the actual operating mode whereas the sub-modes
...
class Repo:
    ...
    def __init__(
        ...
        options -- Set self.options. Any that aren't set will be
            ...
            repo_url -- This is required. It can be an API or web URL
                as long as it ends with username/repo-name (except where
            ...
            caches_path -- Store cached json files here: Specifically,
                ...
                appear in
                "~/.cache/enissue/Poikilos/Enliven/issues". To
    ...
    def getCachedJsonDict(self, url, refresh=False, never_expire=False,
        ...
        URL. For example,
        https://api.github.com/repos/Poikilos/Enliven/issues?q=page:1

        should become something like:
        ~/.cache/enissue/Poikilos/Enliven/
        ...
        https://api.github.com/repos/Poikilos/Enliven/issues/515/timeline

        The reactions to a timeline event are from a URL such as:
        https://api.github.com/repos/Poikilos/Enliven/issues/comments/968357490/reactions
        ...
        if os.path.isfile(c_path):
            ...
            if never_expire is True:
                ...
                expires_s = "never_expire"
            if (refresh is not True) and (is_fresh or never_expire):
                pr_fn(p+"Loading cache: \"{}\"".format(c_path))
                debug(p+"  for URL: {}".format(url))
                debug(p+"Cache time limit: {}".format(max_cache_d_s))
                pr_fn(p+"Cache expires: {}".format(filetime
                                                   + max_cache_delta))
                with open(c_path) as json_file:
                    try:
                        result = json.load(json_file)
    ...
    def show_issue(self, issue, options, refresh=False, never_expire=False,
                   quiet=False):
        '''
        ...
        pr_fn("")
        debug("show_issue...")
        pr_fn("#{} {}".format(issue["number"], issue["title"]))
        # pr_fn(issue["html_url"])
        pr_fn("")
        issue_data = issue
        ...
        spacer = " "
        line_fmt = "{: <" + str(left_w) + "}" + spacer + "{}"
        pr_fn(line_fmt.format("html_url:",  issue["html_url"]))
        pr_fn(line_fmt.format("by:",  issue_data["user"]["login"]))
        pr_fn(line_fmt.format("state:",  issue_data["state"]))
        assignees = issue_data.get("assignees")
        if (assignees is not None) and len(assignees) > 1:
            assignee_names = [a["login"] for a in assignees]
            pr_fn(line_fmt.format("assignees:",
                                  " ".join(assignee_names)))
        elif issue_data.get("assignee") is not None:
            assignee_name = issue_data["assignee"]["login"]
            pr_fn(line_fmt.format("assignee:", assignee_name))
        labels_s = "None"
        ll = None
        ...
        # moved further down: pr_fn(line_fmt.format("labels:", labels_s))
        pr_fn("")
        if markdown is not None:
            pr_fn('"""')
            pr_fn(markdown)
        ...
        if comments > 0:
            pr_fn("")
            pr_fn("")
            pr_fn("({}) comment(s):".format(comments))
        ...
        if this_evts_json_url is not None:
            ...
            # Example:
            # <https://api.github.com/repos/Poikilos/Enliven/
            # issues/202/events>
        '''
        ...
        for evt in data:
            ...
            if login is not None:
                pr_fn(c_prop_fmt.format("from:", login))
            updated_at = evt.get("updated_at")
            if updated_at is not None:
                pr_fn(c_prop_fmt.format("updated_at:",
                                        updated_at))
            body = evt.get("body")
            if body is not None:
                pr_fn("")
                pr_fn(left_margin + '"""')
                pr_fn(left_margin + body)
                pr_fn(left_margin + '"""')
                pr_fn("")
            ...
            if (rename is not None) and ('renamed' not in ignore_events):
                ...
                # pr_fn(left_margin+"renamed issue")
                pr_fn(left_margin+"  from:{}".format(rename.get('from')))
                pr_fn(left_margin+"  to:{}".format(rename.get('to')))
        ...
        pr_fn("")
        pr_fn(left_margin+"labels: {}".format(labels_s))
        ...
        if (closed_by is not None) or (closed_at is not None):
            ...
            if state != "open":
                ...
                if closed_by_login is not None:
                    pr_fn("    (CLOSED{} by {})".format(
                        closet_at_str,
                        closed_by_login
                    ))
                else:
                    pr_fn("    (CLOSED{})".format(closet_at_str))
            if state == "open":
                pr_fn("    (REOPENED)")
            elif closed_at is None:
                pr_fn("    (The closing date is unknown.)")
    ...
    def get_issue(self, issue_no, options):
        '''
...
def main(custom_args=None):
    ...
    if mode == "labels":
        ...
        for label_s in repo.labels:
            ...
            # Enliven/labels/
            # 2016-2018%20unconfirmed%20on%20newer',
            # 'name': '2016-2018 unconfirmed on newer',
