...
if __name__ == "__main__":
    # Help find the module(s)
    sys.path.insert(0, REPO_DIR)
    REPOS_DIR = os.path.dirname(REPO_DIR)
    MTANALYZE_REPO_DIR = os.path.join(REPOS_DIR, "mtanalyze")
    if os.path.isdir(os.path.join(MTANALYZE_REPO_DIR, "mtanalyze")):
        # <repos>/mtanalyze/mtanalyze is the module
        sys.path.insert(0, MTANALYZE_REPO_DIR)

from pyenliven import (  # noqa: E402
    echo0,
)
from pyenliven.forwardfilesync import update_tree  # noqa: E402

from mtanalyze import(  # noqa: E402
    # TRY_SHARE_MT_DIRS,
    get_var_and_check,
)
...
def main():
    ...
    OLD_CONFIG_PATH = os.path.join(configs_path, "EnlivenMinetest")
    CONFIG_PATH = os.path.join(configs_path, "EnlivenLuanti")
    if os.path.isdir(OLD_CONFIG_PATH):
    ...
    MT_MY_GAME_NAME = "subgametest"
    MT_MY_GAME_DIR = os.path.join(MT_GAMES_DIR, MT_MY_GAME_NAME)

    MTG_PATH = None
    mtg_game_name = None
    base_game_path = None
    base_games = ["minetest_game"]
    ...
    if force_update_mtg_enable:
        shutil.rmtree(MT_MY_GAME_DIR)

    # yes | cp -rf $MT_GAMES_DIR/minetest_game/* MT_MY_GAME_DIR"
    # sudo rsync -a $MT_GAMES_DIR/minetest_game/* MT_MY_GAME_DIR"

    try:
        # DOES update minetest_game, but does NOT delete extra mods:
        update_tree(base_game_path, MT_MY_GAME_DIR)
        echo0("Updated \"" + MT_MY_GAME_DIR + "\"...")
    ...
    try:
        ...
        outs = open(os.path.join(MT_MY_GAME_DIR, "game.conf"), 'w')
        ...
        echo0("")
        return 6
    # cmd_string = "sudo mv -f game.conf \MT_MY_GAME_DIR\""
    # shutil.move(tmp_game_conf_path, os.path.join(MT_MY_GAME_DIR, "game.conf"))

    good_dir = os.path.join(MT_MY_GAME_DIR, "mods")
    if os.path.isdir(good_dir):
        echo0("Copied game to " + MT_MY_GAME_DIR)
    else:
        echo0('FAILED to copy game to "{}" ("{}" is missing)'
              ''.format(MT_MY_GAME_DIR, good_dir))
        return 7

    MT_MY_GAME_MODS_PATH = os.path.join(MT_MY_GAME_DIR, "mods")
    MT_MOD_DEST_NAME = "minigamer"
    # MT_MOD_DEST_PATH = os.path.join(MT_MY_GAME_MODS_PATH, MT_MOD_DEST_NAME)

    # if force_update_mtg_mods_enable:
    #     for sub_name in os.listdir(base_game_path):
    #         sub_path = os.path.join(base_game_path, sub_name)
    #         dst_path = os.path.join(MT_MY_GAME_DIR, sub_name)
    ...
    dst_mods_path = MT_MY_GAME_MODS_PATH
